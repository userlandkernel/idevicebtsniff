...
int main (int argc, char *argv[])
{
	...
	err = lockdownd_start_service(lockdown, "com.apple.bluetooth.BTPacketLogger", &service);
	if (err != LOCKDOWN_E_SUCCESS || !(service && service->port)) {
		idevice_free(device);
		printf("Could not start com.apple.bluetooth.BTPacketLogger\n");
		return -1;
	}
	lockdownd_client_free(lockdown);
	...
	if (err != PROPERTY_LIST_SERVICE_E_SUCCESS) {
		idevice_free(device);
		printf("Could not connect to service %s! Port: %i, Err: %i\n", "com.apple.bluetooth.BTPacketLogger", service->port, err);
		return -1;
	}
	lockdownd_service_descriptor_free(service);

	fprintf(stderr, "Got BTPacketLogger: %d\n", service->port);
